...
def _strip_season_suffix(title: Optional[str]) -> str:
    """去除标题中的季集标识（Season N / SxxExx / Sxx），用于兜底匹配。

    Jackett 等索引器的种子通常只写剧名（如 ``Fallout``），而 TMDB 给的
    媒体原标题常带季（如 ``Fallout Season 2``），归一化后二者无法直接字符串
    相交。剥掉季集片段后可让 ``Fallout`` 与 ``Fallout Season 2`` 命中。
    粘连写法（``FALLOUTSEASON``）一并处理。
    """
    if not title:
        return ""
    s = re.sub(r'SEASON\s*\d*', '', title, flags=re.IGNORECASE)
    s = re.sub(r'\s*S\d+E\d+', '', s, flags=re.IGNORECASE)
    ...
    def match_torrent(mediainfo: MediaInfo, torrent_meta: MetaBase, torrent: TorrentInfo) -> bool:
        ...
        if torrent.description:
            ...
            if media_titles.intersection(subtitles) or media_names.intersection(subtitles):
                ...
                return True
        # 兜底：去除季集后缀后再比较标题（仅当类型/年份强校验未拒绝时）
        # 解决媒体原标题带季（如 Fallout Season 2）而种子仅写剧名（Fallout）导致的精确匹配失败。
        media_titles_stripped = {text_tools.normalize_upper(_strip_season_suffix(mediainfo.title)),
                                 text_tools.normalize_upper(_strip_season_suffix(mediainfo.original_title))} - {""}
        media_names_stripped = {text_tools.normalize_upper(_strip_season_suffix(name)) for name in mediainfo.names if name}
        meta_names_stripped = {text_tools.normalize_upper(_strip_season_suffix(torrent_meta.cn_name)),
                               text_tools.normalize_upper(_strip_season_suffix(torrent_meta.en_name))} - {""}
        if meta_names_stripped.intersection(media_titles_stripped) \
                or (media_names_stripped and meta_names_stripped.intersection(media_names_stripped)):
            logger.info(f'{mediainfo.title} 通过标题（去季集后缀）匹配到资源：{torrent.site_name} - {torrent.title}')
            return True
        if torrent_meta.org_string:
            titles_stripped = [text_tools.normalize_upper(_strip_season_suffix(t)) for t in re.split(
                r'[\s/【】.\[\]\-]+', torrent_meta.org_string) if not text_tools.is_english_word(t)]
            if media_titles_stripped.intersection(titles_stripped):
                logger.info(f'{mediainfo.title} 通过标题（去季集后缀）匹配到资源：{torrent.site_name} - {torrent.title}')
                return True
        # 未匹配
