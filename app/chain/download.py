...
class DownloadChain(ChainBase):
    ...
    def _settle_download_success(
        ...
        # 磁力链接无法从种子内容解析文件清单，后台回查下载器补全 downloadfiles
        if torrent.enclosure and torrent.enclosure.startswith("magnet:"):
    ...
    def _add_download_files_from_downloader(
        ...
        # 已存在记录则跳过，避免重复写入（幂等）
        if get_chain_download_history_port().get_files_by_hash(download_hash):
            return False
        ...
        if files_to_add:
            get_chain_download_history_port().add_files(files_to_add)
