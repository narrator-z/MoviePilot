...
class SubscribeChain(MusicSubscribeMixin, InteractionChainMixin, ChainBase):
    ...
    def __build_subscribe_create_context(
        ...
        # 仅有豆瓣/Bangumi ID 而无显式来源时，按对应来源识别，避免跨源订阅被误判为 TMDB 身份
        # （音乐订阅身份由 MusicSubscribeMixin 单独解析，不走此分支）
        if not explicit_identity and mtype != MediaType.MUSIC and options:
            if not media_source and options.get("doubanid"):
                media_source, media_id = MediaSource.Douban, str(options["doubanid"])
            elif not media_source and options.get("bangumiid"):
                media_source, media_id = MediaSource.Bangumi, str(options["bangumiid"])
    ...
    def add(self, title: str, year: str,
        ...
        if error:
            return None, error
        # 豆瓣/Bangumi 源主识别拿不到 tmdbid 时按标题+年份二次识别尽力回填，
        # 仅补 tmdbid/imdbid/tvdbid，保留订阅原有的 doubanid/media_source/media_id。
        self._backfill_tmdbid_by_title_year(context.mediainfo)
    ...
    async def async_add(self, title: str, year: str,
        ...
        # 豆瓣/Bangumi 源主识别拿不到 tmdbid 时按标题+年份二次识别尽力回填，
        # 仅补 tmdbid/imdbid/tvdbid，保留订阅原有的 doubanid/media_source/media_id。
        await self._abackfill_tmdbid_by_title_year(context.mediainfo)
    ...
    def _backfill_tmdbid_by_title_year(self, mediainfo: MediaInfo) -> None:
        """
        ...
        if mediainfo.tmdb_id is not None:
            return
        # 音乐订阅身份由 MusicSubscribeMixin 单独解析，标题+年份二次识别会跨源误判
        if mediainfo.type == MediaType.MUSIC:
            return
        if not mediainfo.title or not mediainfo.year:
            return
        fallback = MediaChain().recognize_media(
    ...
    async def _abackfill_tmdbid_by_title_year(self, mediainfo: MediaInfo) -> None:
        """
        ...
        if not mediainfo.title or not mediainfo.year:
            return
        fallback = await MediaChain().async_recognize_media(
            meta=MetaInfo(f"{mediainfo.title} {mediainfo.year}"),
            mtype=mediainfo.type,
            cache=False,
        )
    ...
    def finish_subscribe_or_not(self, subscribe: Subscribe, meta: MetaBase, mediainfo: MediaInfo,
        ...
        if not subscribe.best_version:
            ...
            if self.__is_truly_completed(subscribe=subscribe, meta=meta, mediainfo=mediainfo) or (
    ...
    def __has_successful_transfer(self, subscribe: Subscribe, meta: MetaBase, mediainfo: MediaInfo) -> bool:
        ...
        try:
            if subscribe.tmdbid:
                rows = TransferHistoryOper().get_by(
                    media_source=subscribe.media_source,
                    media_id=subscribe.media_id,
                    mtype=meta.type.value,
                )
    ...
    def check(self, progress_callback: Optional[Callable[..., None]] = None) -> None:
        ...
        for index, subscribe in enumerate(subscribes, start=1):
            ...
            self._backfill_tmdbid_by_title_year(mediainfo)
            # 对于电视剧，获取当前季的总集数
            ...
            if mediainfo.imdb_id:
                update_data["imdbid"] = mediainfo.imdb_id
            if mediainfo.tvdb_id:
                update_data["tvdbid"] = mediainfo.tvdb_id
            if meta.type == MediaType.MUSIC:
                music_type = getattr(mediainfo, "music_type", None)
                update_data.update({
                    "music_type": music_type,
