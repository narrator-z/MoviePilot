...
class SubscribeQueryOwner(_SubscribeOwnerBase):
    ...
    def _SubscribeChain__media_library_satisfies_subscription(
        self, subscribe: SubscriptionSnapshot, mediainfo: MediaInfo
    ) -> bool:
        """
        ...
        if subscribe.type != MediaType.TV.value or subscribe.season is None:
            return True
        effective_total = self._SubscribeChain__resolve_effective_total_episode(subscribe, mediainfo)
    ...
    def check_and_handle_existing_media(
        ...
        # 如果已下载完毕，执行订阅完成操作
        if exist_flag:
            # 保守守卫：媒体库报「全部存在」时，二次核验其实际返回的集号集合是否真正覆盖
            # 订阅预期集号集合，并确认媒体库集号上限未超过订阅预期总集数。避免两类误判完成：
            #   1) 媒体库误报（集号对上但文件实为其它 / 部分导入）——预期集号未全部在返回集合内；
            #   2) 订阅总集数偏低（媒体库已有超过预期总集数的集）——继续订阅以补齐。
            if not self._SubscribeChain__media_library_satisfies_subscription(subscribe, mediainfo):
