...
# 兼容别名：测试与历史调用方仍通过 TransferHistoryOper() 访问转存历史端口
TransferHistoryOper = get_chain_transfer_history_port
# 兼容别名：测试与历史调用方仍通过 DownloadHistoryOper() 访问下载历史端口
DownloadHistoryOper = get_chain_download_history_port
...
from app.db import subscribe_oper as _subscribe_oper_mod
...
class TransferChain(FileFilterMixin, ScrapeBatchMixin, EpisodeFormatMixin, HistoryMatchMixin, FileKeyMixin,
    ...
    def __json_snapshot(value: Any) -> Any:
        ...
        if value is None:
            return None
        # SimpleNamespace（测试替身或兼容结构）投影为字典，避免 pydantic_core
        # 新版将其序列化为字符串导致 plan 回读校验失败
    ...
    def __build_planning_input(
        ...
        return TransferPlanningInput(
            ...
            need_rename=bool(getattr(target_directory, "renaming", None)) if target_directory else True,
            need_notify=bool(getattr(target_directory, "notify", None)) if target_directory else False,
            overwrite_mode=(getattr(target_directory, "overwrite_mode", None) if target_directory else None),
            episodes_info=tuple(
    ...
    def _ensure_download_files_for_transfer(
        ...
        """
        整理完成时兜底补写 downloadfiles。

        若下载记录缺少文件明细（如磁力链接在添加时元数据尚未就绪），则从下载器回查并写入，
        复用 DownloadChain 的回查补写逻辑（幂等，已存在记录则跳过）。
        """
        ...
        save_path = getattr(torrents[0], "save_path", None) or (str(torrents[0].path) if getattr(torrents[0], "path", None) else None)
        if not save_path:
            return
    ...
    def __perform_transfer(
        ...
        try:
            ...
            # 当规划模块未接管（plan_transfer 返回 None，如无 TMDB id 的兜底或
            # 测中文件路径不存在等场景）时，回退到 29b084b71 的旧整理 ABI：
            # 直接调用 self.transfer，保持向后兼容。
            try:
                transferinfo = self._plan_checkpoint_and_execute(
                    task,
                    source_oper=source_oper,
                ...
                logger.info(
                    "规划检查点未返回，回退旧整理 ABI: %s", planning_error
                )
                transferinfo = self.transfer(
                    fileitem=task.fileitem,
                    meta=cast(MetaBase, task.meta),
                    mediainfo=cast(Union[MediaInfo, MusicInfo], task.mediainfo),
                    target_directory=task.target_directory,
    ...
    def _resolve_subscription_tmdbid(self, task: Any, _unused: Any = None) -> Optional[int]:
        """
        按 task.meta.name 从订阅列表匹配订阅名称，返回该订阅 tmdbid；无匹配返回 None。

        匹配规则：任务名与订阅名双向包含（去除年份/括号等噪声后比较），
        以兼容 "金特务：本色回归 (2026)" 命中 "金特务：本色回归"。
        """
        meta_name = getattr(getattr(task, "meta", None), "name", None)
        if not meta_name:
            return None
        import re as _re
        ...
        norm_task = _norm(meta_name)
        if not norm_task:
            return None
        try:
            subs = _subscribe_oper_mod.SubscribeOper().list() or []
        except Exception as err:
            logger.debug(f"订阅列表查询失败，跳过订阅 tmdbid 兜底：{err}")
            return None
        for sub in subs:
            sub_name = getattr(sub, "name", None) or getattr(sub, "keyword", None) \
                or getattr(sub, "share_title", None)
            if not sub_name:
                continue
            norm_sub = _norm(sub_name)
            if norm_sub and (norm_sub in norm_task or norm_task in norm_sub):
                tmdbid = getattr(sub, "tmdbid", None)
                if tmdbid:
                    return int(tmdbid)
        return None

    def _collect_recognize_ids(self, task: Any, download_history: Any) -> Optional[Dict[str, Any]]:
        """
        收集整理识别所需的媒体身份 ID。

        优先级：download_history 既有身份（media_source+media_id） > tmdbid > 订阅兜底；
        都没有则返回 None，交由文件名识别兜底。
        """
        if download_history is not None:
            media_source = getattr(download_history, "media_source", None)
            media_id = getattr(download_history, "media_id", None)
            if media_source and media_id:
                return {
                    "media_source": media_source,
                    "media_id": media_id,
            ...
            tmdbid = getattr(download_history, "tmdbid", None)
            if tmdbid:
                return {
        ...
        sub_tmdbid = self._resolve_subscription_tmdbid(task, None)
        if sub_tmdbid:
            return {"media_source": "themoviedb", "media_id": str(sub_tmdbid)}
        return None
