...
from app.runtime.extensions.plugin_manager import (
    PluginManager,  # noqa: F401  fork 兼容: re-export 旧路径 app.scheduler.PluginManager
)
...
# fork 兼容: 重新导出 app.scheduler.SystemConfigOper 供遗留调用方与测试桩使用。
# 通过 importlib 挂载以避免静态依赖 app.db 实现层(架构护栏禁止 scheduler 直连 db)。
SystemConfigOper = importlib.import_module("app.db.oper").SystemConfigOper
...
class Scheduler(ConfigReloadMixin, metaclass=SingletonClass):
    ...
    def user_auth(self):
        ...
        if status:
            ...
            self._auth_count += 1
            # 优雅降级：常规认证失败仅以 info 记录并累计次数，避免以 error 刷屏告警
