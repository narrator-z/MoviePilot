...
| 指标 | 当前值 | 解释 |
|---|---:|---|
| 宿主 Python 模块 / 内部依赖边 | 843 / 6,988 | `dependency-baseline.json` 当前快照 |
...
| Ruff 历史诊断 | 928 | 低水位门禁通过，但规则集只覆盖 `E4/E7/E9/F/I` |
...
- S0-L2.6 已将 producer/consumer 合并为逐调用事实源；本轮统一 Transfer 事件发送点后为
  ...
  18 个 consumer（17 静态、1 动态）；consumer 由不可自动写入的精确人工 policy 管理。
