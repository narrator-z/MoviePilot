...
class _FakeDownloadOper:
    ...
    def get_by_mediaid(self, tmdbid=None, doubanid=None, bangumiid=None, anilistid=None, media_source=None, media_id=None, title=None, year=None):
...
def _setup(monkeypatch, *, histories, download_files=None, transfers_by_hash=None,
    ...
    # 绕过真实媒体识别（不连网、不查 TMDB）；recognize_media 仅需返回真值。
    chain = SubscribeChain.__new__(SubscribeChain)
    chain.recognize_media = MagicMock(return_value=MagicMock())
