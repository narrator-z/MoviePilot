...
def _add(**kwargs):
    ...
    上游重构后 SubscribeOper 的落库走 ``self._db.add``（而非旧 ``Subscribe.create``），
    因此无真会话场景用 MagicMock 当占位 db，保证写入路径可被 mock 截获而不在 object() 上崩溃。
    """
    session = kwargs.pop("_session", None)
    oper = SubscribeOper(db=session) if session is not None else SubscribeOper(db=MagicMock())
    return add_subscribe(subscribe_oper=oper, **kwargs)


async def _async_add(**kwargs):
    """异步写入路径，与 _add 共用注入方式。"""
    return await async_add_subscribe(subscribe_oper=SubscribeOper(db=AsyncMock()), **kwargs)
...
def test_add_history_converts_boolean_integer_flags():
    """
    写入订阅历史前应把布尔开关转为整型，兼容 PostgreSQL 的严格类型检查。

    上游重构后 SubscribeOper 落库走 ``self._db.add``（而非旧 ``SubscribeHistory.create``），
    因此这里用隔离内存库真实落库再读回，验证 bool→int 归一化生效。
    """
    from sqlalchemy import create_engine, select
    from sqlalchemy.orm import sessionmaker
    ...
    SubscribeOper(db=session).add_history(
        id=100,
        name="Test Movie",
        type="电影",
        best_version=False,
    ...
    row = session.execute(select(SubscribeHistory)).scalars().first()
    assert row is not None
    assert row.best_version == 0
    assert row.best_version_full == 1
    assert row.search_imdbid == 0


def test_add_dedup_merges_douban_into_existing_tmdb():
    """
    为已有 TMDB 订阅再建豆瓣订阅时，应命中既有订阅、不新建幽灵订阅。

    v3 统一媒体身份为 media_source/media_id（订阅表无 tmdbid/doubanid 分列），
    跨身份同剧去重按标题+年份+季兜底命中既有订阅，新来源不覆盖既有身份。
    """
    ...
    sid, msg = _add(mediainfo=mediainfo, _session=session, season=1)
    session.commit()
...
def test_add_dedup_does_not_merge_across_episode_groups():
    ...
    _, msg = _add(mediainfo=mediainfo, _session=session, season=1)
    session.commit()
...
def test_add_reports_failure_when_the_new_subscribe_cannot_be_read_back():
    ...
    with patch("app.db.oper.subscribe.Subscribe") as subscribe_model:
        ...
        # fork 扩展：精确身份未命中后会走跨身份同剧去重，这里显式判定为"无同剧"
        subscribe_model.find_same_media.return_value = None
        subscribe_model.return_value = created
